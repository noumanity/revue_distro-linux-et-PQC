...
- Scanners de cryptographie au niveau du code source (à distinguer des scanners d'image, voir section 5), ajoutés au titre de la tâche 2 du ticket :
  - SCANOSS Crypto Finder : détecte algorithmes, protocoles, certificats et clés dans le code source et génère un CBOM CycloneDX ; analyse par règles via un moteur ouvert (OpenGrep), avec une approche consciente de la syntaxe (AST) pour limiter les faux positifs ; langages Java, Python, Go et C. SCANOSS est d'abord un outil de SCA (analyse de composition logicielle, par empreintes de code) qui a ajouté la détection cryptographique [12][13].
  - cryptobom-forge (Santander Security Research) : construit un CBOM à partir des résultats d'analyse de CodeQL (Multi-Repository Variant Analysis) [14].
  - CodeQL (GitHub Advanced Security), Semgrep et OpenGrep : moteurs d'analyse statique que l'on peut scripter pour repérer l'usage de cryptographie dans le code et alimenter une génération de CBOM [14].

Note de transparence (tâche 2 du ticket 006) : la première version de cet essai privilégiait les scanners d'image et de système de fichiers, alignés sur la cible de la démo (images au repos). Les scanners orientés code source ci-dessus (par exemple SCANOSS, que l'on trouve par une simple recherche) avaient donc été sous-représentés, alors qu'ils produisent bien des CBOM. La raison n'est pas qu'ils sont hors sujet, mais qu'ils s'appliquent au code source, et non à une ISO ou à un conteneur livré sans source (voir section 5). Ils sont donc des candidats pertinents pour qui dispose du code, moins directement pour notre démo sur images.
...
| SCANOSS Crypto Finder | open source | code source (Java, Python, Go, C) | oui | en croissance | élevée | communautaire et commercial (SCANOSS) |
| cryptobom-forge plus CodeQL | open source | code source (via CodeQL) | oui | en croissance | moyenne | communautaire |
...
12. SCANOSS, dépôt crypto-finder (détection de cryptographie au niveau du code source, sortie CBOM). https://github.com/scanoss/crypto-finder
13. SCANOSS, Crypto Finder, Source-level Cryptography Visibility. https://scanoss.com/crypto-finder-source-level-cryptography-visibility/
14. Santander Security Research, cryptobom-forge (CBOM à partir de l'analyse CodeQL). https://github.com/Santandersecurityresearch/cryptobom-forge
